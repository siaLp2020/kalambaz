...
const makeCategory = (name, icon, prompt, source, clues) => {
  ...
  return {
    ...
    items: source.split(';').filter(Boolean).map((value, index) => {
      ...
      const gallery = galleryManifest[String(categoryIndex)]?.[enName]?.images || []
      return [emoji, faName, enName, description, `generated/items/${categoryIndex}-${index + 1}.mp3`, gallery]
    }),
  }
}
const categories = [
